...
#     new_network <- induced.subgraph(new_network, 
#                                     vids=subcomponent(graph=new_network, v=58))
    plot(new_network, vertex.size=4, edge.arrow.size=0.1)
    my_dom_tree <- dominator.tree(new_network, root=c(58), mode="in")
        
    png("./sample_58.png", width=1000, height=1000)
    plot(my_dom_tree[[2]], vertex.size=5,vertex.label=NULL,edge.arrow.size=0.1)
    dev.off()
    
    
    new_network2 <- induced.subgraph(new_network, 
                                vids=subcomponent(graph=new_network, v=58))
    new_network3 <- induced.subgraph(new_network, 
                                 vids=subcomponent(graph=new_network, v=114))

    new_graph_combine <- graph.union(new_network2, new_network3)

    new_network_real_name <- as.numeric(c(V(new_network2)$name, V(new_network3)$name))
    
    leftover_real_name <- as.numeric(V(new_network)[!V(new_network) %in% new_network_real_name])
    
    leftover_network <- induced.subgraph(new_network, vid=leftover_real_name)

    reunioned_fullnetwork <- graph.union(new_graph_combine, vid=leftover_network)
    
    cbind(V(reunioned_fullnetwork)$name, 
          V(reunioned_fullnetwork),
          as.numeric(V(reunioned_fullnetwork)))
